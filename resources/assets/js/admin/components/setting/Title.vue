<template>
    <div>
        <div>
            <el-button type="primary" size="small" @click="add">添加职称</el-button>
        </div>
...
<script>
    export default {
        ...
        methods:{
            info(){
                axios.get('/admin/setting/titles').then(res => {
                    this.datas = res.data;
                });
            ...
            add(){
                this.$prompt('请输入职称', '添加职称', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    inputPattern: /\S{1,20}/,
                    inputErrorMessage: '请输入的内容不超过20个字符'
                }).then(({ value }) => {
                    axios.post('/admin/setting/title/create', {
                        name: value
                    }).then(res => {
                        let data = res.data;
                        if(data.code === 1){
            ...
            edit(){
                ...
                axios.post('/admin/setting/title/edit', this.obj).then(res => {
                    let data = res.data;
                    // console.log(data);
                    if(data.code === 1){
                        this.$message({
            ...
            deleteFun(id){
                this.$confirm('将删除该分组并将人员分配到其他分组, 是否继续?', '提示', {
                    ...
                    axios.post('/admin/setting/title/delete',{
                        id: id
                    }).then(res => {
                        let data = res.data;
                        // console.log(data);
            ...
            sorting(index, direction){
                if(index+direction>=0 && index+direction < this.datas.length){
                    axios.post('/admin/setting/title/sorting', {
